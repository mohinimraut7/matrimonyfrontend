...
//                     {
//                     label: t("profile.labels.birthTime"),
//                     value: p.birthTime
//                       ? (() => {
//                           let [hours, minutes] = p.birthTime.split(":");
...
//                     {p.siblings
//                       ? `${p.siblings} · ${t("profile.labels.married")}: ${p.siblingsMarried || "—"}`
//                       : "—"}
//                   </p>
//                 </div>
//                 </div>
...
function CardTitle({ children }) {
  return (
    <h2
      className="flex items-center gap-2.5 text-[1.2rem] font-bold text-[#1c1917] tracking-tight mb-4"
      style={{ fontFamily: "'Cormorant Garamond', serif" }}
    >
      <span className="w-[3px] h-5 rounded-sm bg-[#c2852a] flex-shrink-0" />
      {children}
...
/* Converts stored "HH:mm" → "h:mm AM/PM" for display */
...
export default function Profile() {
  ...
  const avatarStr = initials(p.firstName, p.lastName);
  const city =
    [p.currentCity, p.currentState].filter(Boolean).join(", ") || "";
  ...
  // ── Step 2: astro fields are present if any one exists
  const hasAstro =
    p.rashi || p.nakshatra || p.gotra || p.manglik ||
    p.birthCity || p.birthTime;

  const hasPartnerPrefs =
    p.partnerAgeMin || p.partnerAgeMax ||
    p.partnerHeightMin || p.partnerHeightMax ||
    p.partnerMaritalStatus || p.partnerEducation ||
    p.partnerIncome || p.preferredSurname || p.partnerDesc;
      ...
      <div className="min-h-screen px-6 pt-10 pb-16 mt-12">
        <div className="max-w-[1100px] mx-auto flex flex-col gap-5">
          ...
          <div className="bg-white border border-white rounded-[20px] overflow-hidden shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
            <div className="bg-black px-8 py-8 flex items-center gap-8 flex-wrap">
              ...
              <div className="relative flex-shrink-0">
                {p.photos && p.photos.length > 0 ? (
                  ...
                  <div
                    className="w-[110px] h-[110px] rounded-2xl flex items-center justify-center border-2 border-[rgba(243,215,159,0.3)]"
                    style={{
                      background: `linear-gradient(145deg, ${AVATAR_BG[0]}, ${AVATAR_BG[1]})`,
                    }}
                  >
                    <span
                      className="text-[2.8rem] font-bold tracking-[3px]"
                      style={{
              ...
              {/* Name + meta */}
              <div className="flex-1 min-w-[200px]">
                <span className="block text-[0.65rem] font-bold tracking-[0.12em] uppercase text-[#c2852a] mb-1.5">
                  {t("profile.tag")}
                </span>
                <h1
                  className="text-[clamp(1.6rem,3vw,2.1rem)] font-bold text-white tracking-tight leading-tight m-0 mb-1.5"
                  style={{ fontFamily: "'Cormorant Garamond', serif" }}
                >
                  {fullName}
                </h1>
                <div className="flex items-center gap-2 text-white/[0.52] text-[0.95rem] flex-wrap">
                  {age && <span>{age} yrs</span>}
                  {age && p.height && <span className="text-white/25">·</span>}
                  {p.height && <span>{formatHeight(p.height)}</span>}
                  {p.weight && (
                    <>
                      <span className="text-white/25">·</span>
                      <span>{String(p.weight).includes("kg") ? p.weight : `${p.weight} kg`}</span>
                    </>
                  )}
                  {city && (
                    <>
            ...
            {/* Stats strip — caste, marital status, profileFor */}
            <div className="grid grid-cols-3 border-t border-[#f0ede9]">
              {[
                { label: t("profile.labels.caste"),         value: v(p.caste) },
                { label: t("profile.labels.maritalStatus"), value: v(p.maritalStatus) },
                { label: t("profile.labels.profileFor"),    value: v(p.profileFor) },
              ].map(({ label, value }, i) => (
                <div
                  key={label}
                  className={`px-6 py-4 ${i < 2 ? "border-r border-[#f0ede9]" : ""}`}
                >
                  <p className="text-[0.68rem] font-bold tracking-[0.07em] uppercase text-gray-400 mb-1">
                    {label}
                  </p>
                  <p className="text-[0.88rem] font-semibold text-black">{value}</p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5 items-start">
            ...
            <div className="flex flex-col gap-5 order-1">
                ...
                <div className="bg-white border border-[#ede8e1] rounded-2xl p-7 shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                  ...
                  <InfoGrid
                    items={[
                      ...
                      { label: t("profile.labels.manglik"),   value: p.manglik },
                      { label: t("profile.labels.birthCity"), value: p.birthCity },
                      { label: t("profile.labels.birthTime"), value: formatTime(p.birthTime) },
                    ]}
                  />
                </div>
              )}
              ...
              <div className="bg-white border border-[#ede8e1] rounded-2xl p-7 shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                ...
                {/* Father + Mother name row */}
                <div className="grid grid-cols-2 mb-5">
                  <div className="py-3.5 pr-4 border-b border-r border-[#f0ede9]">
                    <p className="text-[0.68rem] font-bold tracking-[0.07em] uppercase text-gray-400 mb-0.5">
                      {t("profile.labels.fatherName")}
                    </p>
                    <p className="text-[0.85rem] font-semibold text-[#1c1917]">
                      {v(p.fatherName)}
                    </p>
                    <p className="text-[0.78rem] text-gray-500 mt-0.5">
                      {v(p.fatherOccupation, "")}
                    </p>
                  </div>
                  <div className="py-3.5 pl-4 border-b border-[#f0ede9]">
                    <p className="text-[0.68rem] font-bold tracking-[0.07em] uppercase text-gray-400 mb-0.5">
                      {t("profile.labels.motherName")}
                    </p>
                    <p className="text-[0.85rem] font-semibold text-[#1c1917]">
                      {v(p.motherName)}
                    </p>
                    <p className="text-[0.78rem] text-gray-500 mt-0.5">
                      {v(p.motherOccupation, "")}
                    </p>
                  </div>
              ...
              <div className="bg-white border border-[#ede8e1] rounded-2xl p-7 shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                ...
                <div>
                  <div className="flex gap-4 items-start py-4 border-b border-[#f0ede9]">
                    ...
                    <div>
                      <p className="text-[0.68rem] font-bold tracking-[0.07em] uppercase text-gray-400 mb-0.5">
                        {t("profile.labels.education")}
                      </p>
                      <p className="text-[0.95rem] font-semibold text-[#1c1917] mb-0.5">
                        {v(p.education)}
                      </p>
                      <p className="text-[0.8rem] text-gray-400">
                        {v(p.fieldOfStudy, "")}
              ...
              <div className="bg-white border border-[#ede8e1] rounded-2xl p-7 shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                ...
                {p.hobbies && p.hobbies.length > 0 && (
                  <>
                    <div className="h-px bg-[#f0ede9] my-4" />
                    <p className="text-[0.68rem] font-bold tracking-[0.07em] uppercase text-gray-400 mb-3">
                      {t("profile.labels.hobbies")}
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {p.hobbies.map((h) => (
                        <span
              ...
              {hasPartnerPrefs && (
                <div className="bg-white border border-[#ede8e1] rounded-2xl p-7 shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                  ...
                  <InfoGrid
                    items={[
                      {
                        label: t("profile.labels.ageRange"),
                        value:
                          p.partnerAgeMin && p.partnerAgeMax
                            ? `${p.partnerAgeMin} – ${p.partnerAgeMax}`
                            : null,
                      ...
                      {
                        label: t("profile.labels.heightRange"),
                        value:
                          p.partnerHeightMin && p.partnerHeightMax
                            ? `${formatHeight(p.partnerHeightMin)} – ${formatHeight(p.partnerHeightMax)}`
                            : null,
                      },
                      { label: t("profile.labels.partnerMaritalStatus"), value: p.partnerMaritalStatus },
                      { label: t("profile.labels.partnerEducation"),     value: p.partnerEducation },
                      { label: t("profile.labels.partnerIncome"),        value: p.partnerIncome },
                      { label: t("profile.labels.preferredSurname"),     value: p.preferredSurname },
                    ]}
                  ...
                  {p.partnerDesc && (
                    <>
                      <div className="h-px bg-[#f0ede9] my-4" />
                      <p className="text-[0.68rem] font-bold tracking-[0.07em] uppercase text-gray-400 mb-2">
                        {t("profile.labels.idealPartner")}
                      </p>
                      <p className="text-gray-500 text-[0.87rem] leading-[1.75]">
                        {p.partnerDesc}
                      </p>
                    </>
                  )}
              ...
              {/* Extra Photos — Step 7 (if more than 1) */}
              {p.photos && p.photos.length > 1 && (
                <div className="bg-white border border-[#ede8e1] rounded-2xl p-7 shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                  <CardTitle>{t("profile.photosTitle")}</CardTitle>
                  <div className="flex flex-wrap gap-3">
                    {p.photos.map((src, i) => (
                      <img
                        ...
                        src={src}
                        alt=""
                        className="w-28 h-28 rounded-xl object-cover border border-[#ece8e1]"
                        style={{ objectPosition: "top center" }}
                      />
                    ))}
                  </div>
            ...
            <div className="flex flex-col gap-5 order-first lg:order-last lg:sticky lg:top-20">
              ...
              <div className="bg-white border border-[#ede8e1] rounded-2xl overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                <div className="bg-[#1c1917] px-5 py-4">
                  <p
                    className="text-[1.05rem] font-bold text-white tracking-tight m-0"
                    style={{ fontFamily: "'Cormorant Garamond', serif" }}
                  >
                    {t("profile.personalDetails")}
                  </p>
                ...
                <div className="px-5 pt-2 pb-5">
                  {[
                    // Step 1 fields
                    { label: t("profile.labels.age"),           value: age ? `${age} years` : null },
                    { label: t("profile.labels.dob"),           value: p.dob ? new Date(p.dob).toLocaleDateString("en-IN") : null },
                    { label: t("profile.labels.gender"),        value: p.gender },
                    { label: t("profile.labels.height"), value: formatHeight(p.height) },
                    { label: t("profile.labels.weight"), value: p.weight ? (String(p.weight).includes("kg") ? p.weight : `${p.weight} kg`) : null },
                    { label: t("profile.labels.bodyType"),      value: p.bodyType },
                    { label: t("profile.labels.complexion"),    value: p.complexion },
                    { label: t("profile.labels.maritalStatus"), value: p.maritalStatus },
                    { label: t("profile.labels.profileFor"),    value: p.profileFor },
                    // Step 2 fields
                    { label: t("profile.labels.currentState"),  value: p.currentState },
                    { label: t("profile.labels.district"),      value: p.district },
                    { label: t("profile.labels.taluka"),        value: p.taluka },
                    { label: t("profile.labels.currentCity"),   value: p.currentCity },
                    { label: t("profile.labels.pincode"),       value: p.pincode },
                    { label: t("profile.labels.caste"),         value: p.caste },
                    { label: t("profile.labels.gotra"),         value: p.gotra },
                    { label: t("profile.labels.manglik"),       value: p.manglik },
                    // Step 5 fields (quick lifestyle refs)
                    { label: t("profile.labels.diet"),          value: p.diet },
                    { label: t("profile.labels.smoking"),       value: p.smoking },
                    { label: t("profile.labels.drinking"),      value: p.drinking },
                  ]
                    ...
                    .map(({ label, value }, i, arr) => (
                      <div
                        key={label}
                        className={`flex justify-between items-baseline py-2.5 gap-2 ${
                          i < arr.length - 1 ? "border-b border-[#f0ede9]" : ""
                        }`}
                      >
                        <span className="text-[0.72rem] text-gray-400 font-normal whitespace-nowrap">
                          {label}
                        </span>
                        <span className="text-[0.8rem] font-semibold text-[#1c1917] text-right">
                          {value}
                        </span>
                      </div>
                    ))}
              ...
              {p.mobile && (
                <div className="bg-white border border-[#ede8e1] rounded-2xl overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.045)]">
                  ...
                  <div className="px-5 py-4">
                    <p className="text-[0.72rem] text-gray-400 mb-0.5">
                      {t("profile.labels.mobile")}
                    </p>
                    <p className="text-[0.88rem] font-semibold text-[#1c1917]">
                      {p.mobile}
                    </p>
                  </div>
                </div>
